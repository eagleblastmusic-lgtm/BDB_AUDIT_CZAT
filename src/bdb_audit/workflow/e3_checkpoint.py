"""Durable E3 blind novelty checkpoint sealing.

E3-X/Y/Z external results are accepted proposals only.  Before any coverage,
gap, cumulative-corpus, or holdout reveal, every accepted blind result and its
pre-reveal discovery records are sealed in canonical history on one exact cut.

Manual external execution does not self-certify stronger independence than the
accepted attempt/knowledge/isolation records already provide.
"""
...
class E3BlindCheckpointSummary:
    campaign_id: str
...
def _accepted_result_for_job(
    ...
    if len(rows) != 1:
        raise ValidationError(
            "E3_BLIND_RESULT_SET_INCOMPLETE",
            (
                f"{job.lane_slot}: expected 1 accepted "
                f"result, got {len(rows)}"
            ),
        )
    return rows[0]


class E3BlindCheckpointService:
    """Seal all E3 blind novelty lanes atomically before reveal."""
    ...
    def __init__(
        ...
        if (
            batch.stage_id != "E3"
            or batch.phase_id != "E3-BLIND"
        ):
            raise ValidationError(
                "E3_BLIND_BATCH_REQUIRED",
    ...
    def _existing(
        ...
        for row in self.store.accepted_records(
            ...
            if not isinstance(attempt, dict):
                continue
            slot = attempt_to_slot.get(
                attempt.get("revision_digest")
            )
            if slot is None:
                continue
            if (
    ...
    def seal(self) -> E3BlindCheckpointSummary:
        ...
        if self.inbox is not None:
            ...
            missing = [
                slot
                for slot, state in (
                    self.inbox.lane_statuses.items()
                )
        ...
        if existing:
            if set(existing) != set(
                ...
                raise ValidationError(
                    "PARTIAL_E3_BLIND_CHECKPOINT_SET"
                )
            seqs = {
                row["accepted_seq"]
        ...
        discoveries_by_slot: dict[
            str, list[CanonicalObject]
        ] = {}
        ...
        for slot in self.batch.lane_slots:
            ...
            discoveries_by_slot[slot] = discoveries
        ...
        seed = ":".join(
            sorted(
                obj.digest
                for obj in checkpoints.values()
            )
        )
        command = CommandEnvelope(
            command_id=_command_id(
                "e3_blind_checkpoint:" + seed
            ),
        ...
        return E3BlindCheckpointSummary(
            ...
            checkpoint_refs={
                ...
                for slot, obj in checkpoints.items()
            },
            discovery_refs={
                slot: tuple(
                    obj.as_ref(
