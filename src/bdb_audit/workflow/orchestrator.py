...
delivery UX, multi-ZIP inbox ingestion, and fail-closed stage progression.
"""
...
class FullAuditOrchestrator:
    ...
    def run_preflight(self, explicit_target_sha: str | None = None) -> PreflightReport:
        ...
        if profile.delivery_profile:
            ...
            checks.append(PreflightCheckResult("Delivery profile", "BLOCKED", "Unknown delivery profile"))
    ...
    def initialize_campaign(
        ...
        """Create or locate the campaign store and record in history index."""
        out_dir = Path(self.settings.output_work_dir).resolve()
        ...
        if store_path:
            ...
            p = out_dir / safe_name / "campaign.sqlite"

        p.parent.mkdir(parents=True, exist_ok=True)
        self.active_store_path = p

        seed = campaign_seed or f"{target_display}_{self.resolved_source.exact_commit_sha if self.resolved_source else 'seed'}"
        if not p.exists() or p.stat().st_size == 0:
            created = self.api.create_campaign(p, seed=seed)
    ...
    def advance_to_next_stage(self) -> dict[str, Any]:
        ...
        # E1 is complete. Moving to E2.
        # Check canonical E2 requirements:
        # In this release, E1 parallel workflow is end-to-end.
        # Subsequent stage automation stops with NEEDS_IMPLEMENTATION or domain gate.
        return {
            "status": "HALTED",
            "current_stage": "E1",
            "next_stage": "E2",
